...
### 1. 环境依赖
*   **Isaac Lab**: 请确保已安装 [Isaac Lab](https://isaac-sim.github.io/IsaacLab/)。
...
**选项说明：**
1.  **[Isaac仿真训练]**: 启动 GPU 并行训练模式（默认开启 4096 个环境）。
2.  **[仿真效果预览]**: 开启图形化窗口，查看练好的模型在仿真中的表现（Sim2Sim）。
3.  **[真机部署运行]**: 连接真实的 Z1 机器人，加载训练产出的 `.pt` 策略模型。
